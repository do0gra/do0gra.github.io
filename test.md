...
Insecurebankv2 is vulnerable Android app made for security enthusiasts and developers to learn the Android insecurities. The app can be found here: https://github.com/dineshshetty/Android-InsecureBankv2. This post details the analysis of the decompiled code and tests that were performed to confirm the findings. 


## Setup
There are two components in the package:
- AndroLab - Server that handles and respond to requests from Insecurebankv2 
- InsecureBankv2.apk - The vulnerable Android app

The lab was setup as follows
...
| Tool name        | Purpose |         
|:-------------|:------------------|
| Genymotion | Provides an Android Emulator that will run the Insecurebankv2 |
| jadx | Android APK decompiler |
| Android Debug Bridge (adb) | Commandline tool to communicate with the Genymotion android device |
...
I started off by decompiling InsecureBankv2.apk using jadx and analysed the source code. I discovered afew vulnerabilities as identified by the developer and tested the hypothesis to confirm my findings.

### Developer Backdoors

In the `LoginActivity` class, a listener is set on the `login` button. When clicked,  the `performlogin()` function.

![login_1](/assets/images/test/login_1.png)


In the performlogin() function, an new intent is created to DoLogin.class and the Username and Password strings are passed 
![login_2](/assets/images/test/login_2.png)


In DoLogin.class, the username and password strings are extracted and saved as this.username and this.password. a AsyncTask is with `doInBackground` method is called that calls `postData`
![login_3](/assets/images/test/login_3.png)

1. Two HTTPpost requests to different endpoints are created - one to `/login` and another to `/devlogin`
2. The username is checked and if it is equal to `devadmin`, perform the HTTP post request to `/devlogin`
3. If the HTTP request returns a valid response, `savecreds()` is called. This will be looked at in the next section: [Exploiting Weak Cryptography Section](#exploiting-weak-cryptography)
![login_4](/assets/images/test/login_4.png)

On the emulator side, I entered `devadmin` as the username and a random password `secretp@55w0rd`. The password is not checked here but this password will be used in explaining the vulnerability in the next section.
![devadmin_login](/assets/images/test/devadmin_login.png) 


Successfully logged in as developer admin in the postlogin page.
![postlogin](/assets/images/test/postlogin.png) 


### Exploiting Weak Cryptography
Looking at the `saveCreds` method, a file `mySharedPreferences` is opened and edited. The username is based64 encoded while the password is passed to `crypt.aesEncryptedString` and the results are written to `mySharedPreferences`
![creds_1](/assets/images/test/creds_1.png) 

Switching over to CryptoClass. it can be seen that the encryption key is harded as `This is the super secret key 123` and initialisation vector is also hard coded as an array of 16 zero hexadecimal bytes. The cryptograhic algorithm implmented is AES in Cipher Block Chaining (CBC) mode.
...
I used cyberchef to decrypt the password
...
I used Frida to hook the two functions to always return false, using the following python script:
...
While looking at the AndroidManifest.xml file, I observed that there is a broadcast receiver named `MyBroadCastReceiver`
...
The approach to take is to trigger the broadcast receiver using Drozer and leak the `textMessage` when `smsManager.sendTextMessage` is called.
...
I used burpsuite as a proxy that intercepts the network traffic.

I then used adb to configure the android emulator's WIFI proxy settings to route http traffic through burpsuite
```cmd
adb shell settings put global http_proxy 192.168.18.7:8080
```

I then clicked on the `change password` button in the application and attempt to change `devadmin` password. As Burpsuite intercepts the http traffic, the credentials was seen as plaintext in the http body.
![http_1](/assets/images/test/http_1.png) 
